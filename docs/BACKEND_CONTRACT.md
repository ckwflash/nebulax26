...
- `POST /api/instances/{id}/solve-all` accepts `{client_request_id, seconds?:90, force?:false}` and returns HTTP 202 with a durable parent `{id,instance_id,status,children:Run[]}`. `client_request_id` is an idempotency key. Three children are created in A/B/C order and execute in parallel under one admitted job and one fenced lease. `NIGHTSHIFT_SOLVER_THREADS` is the total thread budget: eight gives three concurrent scenarios with 3/3/2 solver threads for A/B/C. The allocation uses the entire budget without oversubscribing it; standalone solves can use all eight threads. Each child receives at most 90 seconds (270 aggregate solver-seconds), and persisted allocation deadlines prevent restart from granting extra time. Shorter budgets of 1–90 seconds are accepted. Recovery comparisons retain their serial, shared 90-second budget.
- Completed scenario results and proven infeasibility are cached durably by dataset content ID, scenario, budget, effective baseline/overrides/bookings/preferences, rule version and solver version. Cache hits expose `cache_hit`, `cached_from_id`, `cached_elapsed_seconds`, and zero `elapsed_seconds`; schedules are independently rechecked before reuse. Partial hits solve only missing scenarios. All-hit requests return a completed batch without entering the solver queue. Uploads reuse the cache; the explicit Re-solve button sends `force:true`. Unknown, failed and checker-error results are not cached. Existing completed children remain unchanged during recovery.
...
The shared provider exposes `history`, `refreshHistory()`, `solveAll()` and `loadSavedRun(id)`. The Use A/B/C buttons in Scenario results call `switchScenario`, which reuses saved results and immediately adopts the selected feasible plan. A missing result is solved and adopted after validation. Controls remain visible while saving; concurrent clicks are blocked, and a rejected adoption restores the active approved plan with a visible error. Selecting the active plan is a no-op. What-if/recovery previews and historical-version inspection retain their explicit adoption controls in a separate row. Dataset switching aborts obsolete polling; reopening a pending dataset resumes polling, and reopening a completed dataset requires no new solve. The Dataset library searches saved names/IDs and restores the approved plan first, or a saved preview when no plan is approved. This public demo's library is shared, for non-sensitive data.
...
Solving, improving, or inspecting a historical/what-if run does not itself change approval. A/B/C scenario selection explicitly invokes adoption in the same action. Reload restores the last successfully adopted plan.
