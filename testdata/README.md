...
Reports produced before the September 19 closure and scoring corrections were removed from the submitted tree because they do not validate the current rules. The current compact matrix remains in `results/scoring-fix/`. Case 06 now has two weeks: its Live jobs must occupy different weeks. In cases 03/04, B may return a complete diagnostic overrun, but it must fail validation and refuse export.
...
| `03_sharing_limit.zip` | 5 / 5 | Fifth compatible job exceeds four-job sharing limit | 70 | Deadline infeasible | 70 |
| `04_exclusive_mixes.zip` | 3 / 3 | PM isolation and PC incompatibility | 210 | Deadline infeasible | 210 |
...
| `08_priority_pressure.zip` | 54 / 192 | Public network with changed priority weights | ≥12670 lower bound | 30 | 3870 |
| `09_double_network.zip` | 108 / 384 | Four-line scalability and independent components | 275.8 | 60 | 125.4 |
...
- **Independent answers:** workload arithmetic, sharing limits and available weeks establish bounds without invoking the planner. Case 08 also uses a resource-free C bound by enumerating contract finish weeks and the 0–2 ECLO accesses each activity would need. Valid schedules attaining those bounds prove their primary scores optimal.
...
- **Relationship tests:** CSV row shuffling, ID renaming and a 28-day date shift must preserve results. More supply cannot waive weekly closure exclusions. A longer horizon restores A/C feasibility in case 10 while B's unchanged hard deadline remains impossible.
...
New runs write to the ignored `.nightshift/test-runs/local/` or `.nightshift/test-runs/hosted/`, preserving the published evidence. For a focused rerun, add `--only 07_eclo_window --scenarios C --out .nightshift/test-runs/focused`. Without `--out`, the runner replaces that mode's local summary with the selected run. The input generator does not delete solver results. Regenerate the outer bundle after editing this guide if distributing it.

## Verification artifacts

The outer collection includes `TEST_RESULTS.md` and the current compact matrix under `reports/`. Superseded matrices and full historical schedules are excluded.

- `results/scoring-fix/REPORT.md` and `report.json`: published per-scenario scores, bounds, statuses, timings and coverage.
- `.nightshift/test-runs/<mode>/`: newly generated summary reports; hosted rows include saved run IDs.
- `.nightshift/test-runs/<mode>/<case>/<scenario>/`: newly generated schedule CSVs, timing witness, solver report and exact three-CSV export ZIP for feasible cases.
- `TEST_RESULTS.md`: combined outcome and timing summary.

For every feasible run, the runner independently revalidates the returned schedule, checks per-activity delivered work, compares the known score, round-trips CSV serialization and verifies the export contains exactly `SCHEDULE_ACCESS.csv`, `SCHEDULE_OCCUPANCY.csv` and `RESULTS.csv`. It also moves the two Live jobs into the same week to check that cross-line closure conflicts are rejected. Hosted infeasible runs must refuse export with HTTP 409.
