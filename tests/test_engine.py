...
def test_public_routes_and_bounds(public):
    ...
    assert public.lower_bounds()["A"] == 25.2
...
def test_reference_regression(public):
    ...
    assert report["score"] == 48.3
    assert report["soft_scores"]["overrun_days_total"] == 28


@pytest.mark.parametrize("scenario,score", [("A",25.2),("B",30),("C",25.2)])
...
def synthetic(seed):
    """Known feasible: each of six contracts can take its own opportunity/week."""
    ...
    tables["parameters"]=[dict(key="horizon_start",value="2027-01-04"),dict(key="horizon_weeks",value=5)]
    ...
    for i in range(6):
        ...
        activities.append(dict(activity_id=aid,contract_number=cid,activity_type="Renewal",start_location_id=parts[first]["sector_id"]+":"+bound,end_location_id=parts[last]["sector_id"]+":"+bound,total_accesses=rng.randint(1,3),planned_start_date=rng.choice(["2027-01-04","2027-01-11"]),predecessor_activity_id="",activity_priority=rng.randint(1,3)))
...
def test_c_forced_eclo_stays_continuous():
    ...
    def harder(rows):
        for row in rows:
            row.update(total_accesses='6',planned_start_date='2027-01-04')
